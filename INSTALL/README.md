...
- Enrutador y middlewares: `net/http` + `http.ServeMux` (Go 1.22+) con middlewares para JWT, request-id, rate limiting y métricas.
...
- Streaming: WebSocket + HTTP streaming con fallback CLI en la extensión VS Code.
- Observabilidad: métricas JSON y endpoint Prometheus (`/metrics/prometheus`).
- Configuración operativa: soporte por variables de entorno + archivo JSON (`CONFIG_FILE`) y recarga en caliente vía endpoint administrativo.
...
- `api/internal/function/*` — módulos por capacidad con separación `service/transport/domain` (ej. chat, review, security, runtime_config, etc.).
- `api/internal/server` — wiring de dependencias, registro de rutas y cadena de middlewares.
...
# Gate de cobertura en paquetes críticos (default >=70)
cd .. && make test-coverage-gate

# Validación de tamaño de artefacto de extensión (<5MB)
cd vscode-extension && npm run size:check
```
...
- `POST /api/admin/config/reload` — recarga configuración runtime sin reiniciar proceso (JWT)
