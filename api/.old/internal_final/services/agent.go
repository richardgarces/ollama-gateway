...
func (a *AgentService) safeReadFile(input string) string {
	absPath, err := filepath.Abs(input)
	if err != nil {
		return "ruta inválida"
	}
	allowedRoot, _ := filepath.Abs(a.repoRoot)
	if !strings.HasPrefix(absPath, allowedRoot+string(os.PathSeparator)) && absPath != allowedRoot {
		return "acceso denegado: fuera del directorio permitido"
	}
	data, e := os.ReadFile(absPath)
...
func (a *AgentService) Execute(prompt string) string {
	toolsDef := `
Herramientas:
1. get_time -> devuelve hora
2. read_file -> lee archivo del proyecto
...
`
	...
	resp, err := a.ollamaService.Generate("qwen2.5:7b", fullPrompt)
	if err != nil {
		return err.Error()
	}
	...
	case "get_time":
		return time.Now().Format(time.RFC3339)
	case "read_file":
		return a.safeReadFile(ar.Input)
	default:
		if ar.Response != "" {
			return ar.Response
		}
